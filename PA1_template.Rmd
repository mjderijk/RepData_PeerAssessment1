...
The interval where the maximum number of steps across the 5-minute intervals occurs is `r filter(intervalDF, average == max(average))$interval`:
...
1. mean: `r mean(imputedDailyTotals$dailyTotal)`
2. median: `r median(imputedDailyTotals$dailyTotal)`
